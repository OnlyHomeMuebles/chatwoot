...
`Llm::BaseAiService`); namespace `OnlyHome::Copilot`, sin tocar `enterprise/`.

## Piezas

| Componente | Archivo | Rol |
|---|---|---|
| Agente copiloto | `app/services/only_home/copilot/agent.rb` | `Agents::Agent` que asiste al agente humano; lee la conversación antes de sugerir |
| Herramienta de contexto | `app/services/only_home/tools/get_conversation_tool.rb` | Lee los mensajes recientes de la conversación (Application API) |
| Servicio de sugerencia | `app/services/only_home/copilot/suggestion_service.rb` | Punto de entrada: dada la conversación + la consulta del agente, devuelve la sugerencia |

## Cómo se usa

```ruby
service = OnlyHome::Copilot::SuggestionService.new
...
| Ubicación | `enterprise/` | open source (`app/services/only_home/`) |
| Namespace | `Captain::Copilot` | `OnlyHome::Copilot` |
