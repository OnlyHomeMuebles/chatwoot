  ...
  describe 'mensajes con adjuntos' do
    ...
    it 'un mensaje solo con foto (sin texto) SI hace que el agente responda, con las imagenes para el OCR' do
      expect { described_class.new(solo_imagen_payload).process }
        .to have_enqueued_job(Helic3::ProcessConversationJob)
        .with(account_id: 1, conversation_id: 7, content: '',
              imagenes: ['https://cdn.chatwoot.test/foto-dano.jpg'], hay_adjuntos: true)
    end
    ...
    # B3 (revision de Jhan, 23-sep): un adjunto que NO es imagen (nota de voz,
    # PDF, ubicacion), sin texto, tambien debe activar el respaldo -- antes se
    # invocaba al runner con una cadena vacia.
    it 'un mensaje SOLO con un adjunto que no es imagen (nota de voz) tambien hace que el agente responda' do
      solo_audio = incoming_payload.merge(
        content: '', attachments: [{ file_type: 'audio', data_url: 'https://cdn.chatwoot.test/nota.ogg' }]
      )

      expect { described_class.new(solo_audio).process }
        .to have_enqueued_job(Helic3::ProcessConversationJob)
        .with(account_id: 1, conversation_id: 7, content: '', imagenes: [], hay_adjuntos: true)
    end
  end
end
