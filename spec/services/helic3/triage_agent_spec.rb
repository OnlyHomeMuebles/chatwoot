...
RSpec.describe Helic3::TriageAgent do
  describe '.build' do
    ...
    it 'lleva la herramienta de escalamiento a humano (para pedidos explícitos o casos sensibles)' do
      expect(agent.tools.map { |t| t.class.name }).to include('Helic3::Tools::HumanHandoffTool')
    end
  ...
  describe 'registro de handoffs' do
    ...
    let(:faq)          { Helic3::FaqAgent.build(model: 'gpt-4.1-mini') }
    let(:pqrs)         { Helic3::PqrsAgent.build(model: 'gpt-4.1-mini') }
    let(:logistica)    { Helic3::LogisticaAgent.build(model: 'gpt-4.1-mini') }
    let(:cotizaciones) { Helic3::CotizacionesAgent.build(model: 'gpt-4.1-mini') }
